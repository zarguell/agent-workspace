...
from secrets_store import encrypt_if_stable
...
def _new_canvas_key():
    ...
    Stored Fernet-encrypted when SECRETS_MASTER_KEY is stable (so DB
    readers and backups can't see it); plaintext under the ephemeral dev
    fallback — same generated value as before, just encrypted at rest."""
...
class Workspace(Base):
    ...
    # If True, the reconciler keeps the workspace's PVC/namespace on delete
    # (hibernation-style teardown) so data survives for manual recovery.
    preserve_pvc = Column(Boolean, nullable=False, default=False, server_default=text("false"))
    ...
    # Encrypted at rest (Fernet) when SECRETS_MASTER_KEY is stable; legacy
    # pre-encryption rows keep plaintext and the reconciler passes those
    # through verbatim. VARCHAR(255) fits Fernet tokens (~120 chars).
    canvas_api_key = Column(String(255), nullable=False, default=_new_canvas_key)
    canvas_secret_key = Column(String(255), nullable=False, default=_new_canvas_key)
    ...
    # Per-workspace token the pod presents as X-Workspace-Token on
    # POST /api/internal/usage; identity is derived from the workspace
    # owner. Generated by the reconciler (secrets.token_urlsafe(32));
    # NULL until the reconciler first ensures it.
